...
const Contact = () => {
  ...
  return (
    <section className='flex flex-col px-52 gap-32 mt-28 z-20'>
        {/* Header */}
        ...
        <div className='flex gap-32'>
            {/* FORM */}
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col w-4/6' action="">
                ...
                <div className='flex flex-col h-auto border-t border-b py-12 gap-7'>
                    ...
                    <textarea className='w-4/6 ml-9 text-xl text-white bg-transparent border-none outline-none p-3 resize-none' cols="30" rows="5" id='message' name='message' type="text" placeholder='Hello Alan, can you help me with... *' {...register('message', { required: true })}></textarea>
                </div>
                {/* Submit Button */}
                <div className='relative flex flex-col items-center justify-end py-12'>
                    ...
                    <Magnetic>
                        <button onClick={onSubmit} className='w-[150px] h-[145px] bg-gradient-to-br from-neutral-300 to-neutral-500 rounded-full border-slate-500 z-10 flex justify-center items-center shadow-md'>
                            <p className='text-white font-bold hover:scale-125 hover:font-extrabold transition-all duration-300 cursor-pointer'>Send</p>
                        </button>
                    </Magnetic>
                </div>
            </form>
