...
const Project = ({index, title, setModal}) => {
    ...
    return (
        <div onMouseEnter={() => {setModal({active: true, index})}} onMouseLeave={() => {setModal({active: false, index})}} className='group flex flex-col sm:flex-row w-full text-white justify-between items-center py-12 px-24 border-t border-gray-200 cursor-pointer transition-all duration-200 hover:opacity-50'>
